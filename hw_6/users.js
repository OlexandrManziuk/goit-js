...
const getUsersWithGender = (users, gender) =>
  users
    .filter((filteredUsers) => filteredUsers.gender === gender)
...
const getUserWithEmail = (users, email) =>
  users.find((findedUser) => findedUser.email === email);
...
const getUsersWithAge = (users, min, max) =>
  users.filter(
    (filteredUsers) => filteredUsers.age >= min && filteredUsers.age <= max
  );
console.log("Task 6:", getUsersWithAge(users, 20, 30)); // [объект Ross Vazquez, объект Elma Head, объект Carey Barr]
console.log("Task 6:", getUsersWithAge(users, 30, 40)); // [объект Moore Hensley, объект Sharlene Bush, объект Blackburn Dotson, объект Sheree Anthony]
